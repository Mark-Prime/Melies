...
impl Clip {
  pub fn new(event: Event, settings: &Value) -> Clip {
    ...
    match &event.value {
      Killstreak(val) => {
        let mut start_tick =
          &event.tick - settings["recording"]["before_killstreak_per_kill"].as_i64().unwrap() * val;
    ...
    match &event.value {
      Bookmark(bm) => {
        ...
        if !bm.contains("clip_start") {
          ...
          if
            let Some(min_tick_between_clips) =
      ...
      Killstreak(killstreak_value) => {
        ...
        let new_start =
          &event.tick -
          settings["recording"]["before_killstreak_per_kill"].as_i64().unwrap() * killstreak_value;
