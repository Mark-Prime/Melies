<script>
  ...
  function increment(num, oldIndex = index) {
    ...
    if (
      saved.includes(videos[index]?.path) ||
      deleted.includes(videos[index]?.path)
    ) {
...
<Modal
  ...
  {toggle}
  {enabled}
  large={videos.length > 0}
  max_width={videos.length > 0 ? "1425px" : null}
>
  <div class="video-modal">
    <div class="input-group">
      <Input title="Recordings Folder" color="tert" bind:value={folder} />
      <button class="btn btn--tert" onclick={load_files}> Open </button>
    </div>
    {#key doRerender}
      {#if videos.length > 0}
        ...
        <div class="video-player">
          ...
          <div class="side-buttons">
            <div class="layers">
              Open Layer
              <button
                onclick={() => open_file(videos[index].layers[defaultVideo])}
                class="btn"
                >{defaultVideo[0].toUpperCase() +
                  defaultVideo.slice(1).toLowerCase()}</button
              >
